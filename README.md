...
- Live Rich Message drafts show the current request phase and elapsed time while ordinary assistant requests run. A bounded OpenRouter structured-output planner chooses direct generation or gives the main model an ordered skill plan; explicit commands remain deterministic.
...
The model chooser is per capability: general chat, image understanding, video understanding, image generation, speech generation, transcription, and video generation each have an independent per-bot selection and upstream API. OpenRouter and AI Hub have separate tabs and catalogs; equal model IDs cannot be confused across providers. Teleforge caches OpenRouter's authenticated `/models/user` catalog and AI Hub's authenticated `/models` catalog separately for every bot. OpenRouter models marked as requiring additional identity attestation are omitted. The fuzzy picker searches compatible models by name, ID, description, and modality, ranks equal matches newest-first, and renders only the best 60 results so the Web App stays responsive.
...
Commands can start with either `/` or `-`, and command names are case-insensitive—for example, `-SEARCH current Istanbul transit news`. In groups, a dash command is recognized without requiring a bot mention. Guest image, audio, and video generation immediately posts a pending Rich Message and then edits that same guest result with the completed media. Generated bytes are exposed to Telegram at an unguessable, in-memory HTTPS URL for at most ten minutes; the cache is bounded to 50 MiB per item and 128 MiB total. Attachment-dependent guest operations remain subject to the media made available by Telegram in the guest update.
...
execution instructions, and an allowlisted skill list; the backend intersects that
list with admin-enabled capabilities. A structured refusal is returned directly in
...
timeouts, rate limits, unsupported endpoints, and malformed JSON are logged and fall
back to the ordinary assistant/tool loop, so the planner cannot take the bot offline.
