...
- fal.ai queue and direct endpoints for image, speech, music/audio, video, and transcription models. Endpoint-specific input fields, default parameters, and output JSON pointers are configured in YAML; submission, polling, error handling, result extraction, secure media download, catalog selection, and encrypted per-bot credentials are shared.
