...
- Live reply-linked Rich Messages retain the completed processing steps while assistant and media-generation requests run; generation steps disclose the exact effective prompt and model, and guest mode updates its pending inline result. A bounded OpenRouter structured-output classifier selects actions without producing or rewriting downstream prompts; explicit commands remain deterministic.
