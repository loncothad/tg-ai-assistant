...
- Live reply-linked Rich Messages show the current request phase and elapsed time while assistant and media-generation requests run; guest mode updates its pending inline result. A bounded OpenRouter structured-output planner chooses direct generation or gives the main model an ordered skill plan; explicit commands remain deterministic.
...
The model chooser is per capability: intent processing, its fallback, general chat, image understanding, video understanding, image generation, speech generation, transcription, and video generation each have an independent per-bot selection. Intent models are restricted to OpenRouter text models advertising structured-output support; the primary and fallback selections are stored independently for every bot. OpenRouter and AI Hub have separate tabs and catalogs for the remaining capabilities; equal model IDs cannot be confused across providers. Teleforge caches OpenRouter's authenticated `/models/user` catalog and AI Hub's authenticated `/models` catalog separately for every bot. OpenRouter models marked as requiring additional identity attestation are omitted. The fuzzy picker searches compatible models by name, ID, description, and modality, ranks equal matches newest-first, and renders only the best 60 results so the Web App stays responsive.
...
list with admin-enabled capabilities. The same small-model prompt selects inline or
